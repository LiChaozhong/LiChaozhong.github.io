...
document.addEventListener('DOMContentLoaded', () => {
    ...
    venueElem.innerHTML = pageContent.mainVenue.replace(/\n/g, '<br>');
...
${avatar}
            ...
            console.error('获取嘉宾列表失败，降级使用模拟数据:', error);
            // 降级使用原本留在 content.js 里的 guestData
            guestList.innerHTML = '';
            guestData.forEach(guest => {
                ...
                const tagsHtml = guest.tags.map(tag => {
                    const isAvailable = tag.includes('待认领');
...
${guest.asciiAvatar}
                    ...
                    <h4 class="text-xl font-bold font-mono text-white mb-3 group-hover:text-cyber-primary transition-colors text-center">${guest.name}</h4>
                    <div class="flex flex-wrap gap-2 mb-4 justify-center">
                        ${tagsHtml}
                    </div>
                    <p class="text-cyber-text text-sm mb-6 flex-grow text-center leading-relaxed">${guest.hobbies}</p>
                    <a href="${guest.profileLink}" target="_blank" class="font-mono text-sm text-cyber-primary hover:text-white flex items-center gap-1 transition-all group-hover:-translate-y-1 mt-auto">
